...
**Confirmé en implémentation (Stories 4.4.1 et 4.4.2)** : MixDeck ne ciblant que macOS, seul VST3 est isolé (process dédié `MixDeckPluginWorker` par instance, `juce::ChildProcessCoordinator`/`Worker`) ; AU reste en process principal comme anticipé. Compromis assumé avec Julien : l'éditeur d'un plugin VST3 isolé s'ouvre en fenêtre indépendante réelle plutôt qu'incrusté (incrustation cross-process non réalisable sur macOS sans API privées fragiles). Le streaming audio réel (4.4.2) passe par deux `juce::AbstractFifo` lock-free préallouées + un thread de pompe dédié côté principal, jamais d'accès direct au canal IPC depuis le thread audio — un plugin VST3 qui plante ou décroche ne bloque ni ne crashe le thread audio, le signal sec continue de passer.
**Statut** : Accepté.
