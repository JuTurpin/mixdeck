...
Voir `docs/progress.md` pour le détail story par story. Prochaine étape : Story 5.2 (métadonnées, tags, crates, cue points). Dépôt git initialisé (branche `main`), poussé en privé sur `github.com/JuTurpin/mixdeck` ; JUCE et SoundTouch sont vendorisés en submodules (`native/engine/JUCE`, `native/engine/SoundTouch`).
...
**Prochaine action (voir `progress.md`)** : Epic 5 (bibliothèque de sons) terminé, Story 5.3 (cue points) validée. Prochaine étape à discuter avec Julien : Epic 6 (build local), le dernier lot fonctionnel restant.
