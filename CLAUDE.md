...
**Epic 1 ("moteur audio standalone JUCE"), Epic 2 ("intégration Electron"), Epic 3 ("pitch avancé & BPM"), Epic 4 ("plugins VST3/AU"), Epic 5 ("bibliothèque de sons") et Epic 6 ("build local & lancement", périmètre réduit à la Story 6.1) sont terminés — toutes les epics fonctionnelles de la roadmap sont désormais achevées.** Epic 1 : les 4 stories (lecture 2 pistes, Mixer/crossfader, filtre résonant, pitch vitesse liée) validées à l'oreille. Avant l'Epic 2, une revue d'architecture a affiné le découpage (ADR-013 à ADR-016 : Engine API, couche Controller, modèle événementiel, machine d'état des Decks — voir `docs/decision.md`). Epic 2 : chaîne complète React → Controller → IPC → Bridge → moteur JUCE opérationnelle, vraie UI (Deck/Mixer/Filtre, calquée sur l'export Claude Design), fenêtre sans cadre natif (`frame: false`, fermer quitte toute l'app — ADR-010), modèle événementiel (ADR-015) poussé par le process principal, et checklist de robustesse/performance (Story 2.7) passée sans souci notable. Epic 3 : time-stretch indépendant (SoundTouch, ADR-006), détection BPM à l'import (`BpmAnalyzer`, BPM effectif affiché dans l'en-tête du Deck) et synchronisation automatique entre decks — un bouton Sync par deck en **verrou continu** (pas ponctuel, revu en cours de route sur demande de Julien après test) : tant qu'il est actif, tout changement de pitch/BPM de l'autre deck réapplique le pitch de celui-ci ; reprendre la main sur le slider désengage le verrou. Ce verrou continu n'existe que côté Electron — le harnais standalone JUCE garde un Sync ponctuel (écart assumé). **Toutes les stories validées par Julien.** Epic 4 (plugins VST3/AU, 7 stories 4.0-4.4.2) est également **terminé** — hors backlog 4.5 (VST2/FST, non priorisé, ADR-003) : édition via GUI native incrustée (ADR-011), scan automatique + sélection manuelle (ADR-004), chaîne d'effets réelle par deck et bus master (`PluginChain.h/.cpp`, lock-free — `std::atomic<const ChainSnapshot*>` + générations conservées côté contrôle, PAS `std::atomic<std::shared_ptr<T>>` : non implémenté par le libc++ d'Apple malgré le C++20, piège à ne pas retenter), et isolation hors-process pour VST3 (ADR-005) avec streaming audio réel — un plugin VST3 instable ne peut plus faire planter MixDeck. Un process dédié `MixDeckPluginWorker` par instance VST3 (`native/engine/plugin-worker/`), piloté par `IsolatedPluginHost` (`juce::ChildProcessCoordinator`, protocole `PluginWorkerProtocol.h/.cpp`) ; le thread audio ne touche jamais l'IPC directement (deux `juce::AbstractFifo` lock-free préallouées + thread de pompe dédié). AU reste en process principal, incrusté dans la fenêtre Electron comme avant ; l'éditeur d'un VST3 isolé s'ouvre en vraie fenêtre indépendante (pas d'incrustation cross-process possible sur macOS sans API privées). Point d'attention réutilisable : une vue native incrustée (AU) se compose au-dessus de tout le DOM Electron indépendamment du z-index CSS — toute UI de contrôle pour une vue incrustée doit être native (`PluginChain::EditorHost`), pas dessinée en React. Autre point d'attention (bug trouvé après coup, pendant la Story 5.1, et corrigé) : sur une fenêtre native ajoutée via `addToDesktop` (pas un `DocumentWindow`), le bouton natif de fermeture n'a aucun effet tant qu'on ne surcharge pas `Component::userTriedToCloseWindow()` — comportement par défaut silencieux en build Release ; voir `PluginWorkerMain.cpp::WorkerEditorWindow`.

**Epic 5 ("bibliothèque de sons", 3 stories 5.1-5.3) est également terminé** — entièrement côté Node/Electron (`apps/electron-ui/src/main/library.ts`), **aucun fichier C++ touché sur tout l'epic** (pas de traitement de signal, `Deck::getPosition()`/`seek()` existants suffisent même pour les cue points). SQLite via `better-sqlite3` (module natif, reconstruit contre l'ABI Electron via `@electron/rebuild`, voir commande plus bas — même piège que le Bridge) : scan/import de dossiers avec lecture de tags (`music-metadata`, ESM-only, import dynamique depuis le process main en CJS ; expose `common.bpm`/`common.key`, ce dernier converti en notation Camelot par une table de correspondance à 24 entrées, `library.ts::toCamelotKey`), crates (tags multiples par piste, CRUD minimal, filtre + recherche côté client), cue points (8 hot cues par Deck, persistés par **chemin de fichier** — pas de clé étrangère vers la bibliothèque, fonctionnent aussi hors scan). Point d'architecture réutilisable (Story 5.3) : ni `Deck.tsx` ni `LibraryPanel.tsx` ne savaient nativement "quel fichier est chargé sur quel deck" (deux `DeckController` séparés) — résolu en remontant l'info à `App.tsx`, exactement le même mécanisme que `DeckSyncInfo` (Story 3.3, bpm/pitch entre les deux Deck), sans toucher au natif ni à l'événementiel (ADR-015). Convention actée avec Julien pendant l'epic : un découpage de story en cours d'implémentation (comme 4.4→4.4.1/4.4.2, ou 5.2→5.2+5.3 ici) doit être reflété dans `docs/roadmap.md` — les morceaux encore à faire vont temporairement dans une section "Epic N BIS", promus dans la section principale de l'epic une fois traités (voir historique de `roadmap.md`).

**Epic 6 ("build local & lancement") est terminé, avec un périmètre volontairement réduit à la Story 6.1** — `electron-builder` (`apps/electron-ui/electron-builder.yml`) empaquette un vrai `.app` autonome, lançable par double-clic sans dépendre de `npm run dev`. Le Bridge natif (`mixdeck_bridge.node`) et `MixDeck Plugin Worker.app` ne sont pas des modules npm (construits par `native/engine/CMakeLists.txt`) — copiés hors de l'asar via `extraResources`, contrairement à `better-sqlite3` qui est un vrai module npm et nécessite `asarUnpack` (son binaire natif ne peut pas être chargé depuis l'intérieur de l'archive asar). `mac.identity: null` désactive la recherche/signature automatique d'un certificat Developer ID par electron-builder (aucun, usage perso ADR-010). `src/main/index.ts::createNativeEngine()` bascule la résolution des deux chemins natifs sur `app.isPackaged` (dev inchangé, empaqueté → `process.resourcesPath`). **Choix de Julien le 2026-07-17** : réduire Epic 6 à cette seule story une fois le packaging fonctionnel validé, plutôt que d'enchaîner sur la signature ad-hoc réelle (`codesign --sign -`) et le script one-liner de build — les deux repoussés dans la section "Addon" de `roadmap.md` (pas un blocage technique, une priorisation ; un clic droit → "Ouvrir" suffit à passer Gatekeeper en attendant). Première release GitHub du projet à cette occasion : tag `v0.1.0-beta`.

Voir `docs/progress.md` pour le détail story par story. Dépôt git initialisé (branche `main`), poussé en privé sur `github.com/JuTurpin/mixdeck` ; JUCE et SoundTouch sont vendorisés en submodules (`native/engine/JUCE`, `native/engine/SoundTouch`).
...
Depuis la Story 5.1 (ADR-007), `better-sqlite3` (bibliothèque de sons, `src/main/library.ts`) est lui aussi un module natif à reconstruire contre l'ABI Electron, même piège que le Bridge ci-dessus — mais reconstruit **en place** (contrairement au Bridge, pas de dossier séparé : `better-sqlite3` n'est utilisé que par Electron dans ce projet, jamais par un script Node autonome) :
```
cd apps/electron-ui
npx electron-rebuild -f -w better-sqlite3
```
À relancer après tout `npm install` qui touche `better-sqlite3` ou change la version d'Electron.
...
Depuis la Story 6.1, un vrai `.app` local se génère avec `electron-builder` (`apps/electron-ui/electron-builder.yml`) — nécessite d'abord le Bridge et `MixDeckPluginWorker` construits pour l'ABI Electron (commande ci-dessus) :
```
cd apps/electron-ui
npm run build              # electron-vite build → out/
npm run dist                # electron-builder --mac → dist/mac-arm64/MixDeck.app
```
Signature non traitée (`mac.identity: null`, voir ADR-010) — un clic droit → "Ouvrir" peut être nécessaire au premier lancement du `.app` généré.
...
│   ├── src/main/           # fenêtre (sans cadre natif, Story 2.5) + relai IPC vers le Bridge (Story 2.4) + commandes fenêtre + engineEvents.ts (pump événementiel, Story 2.6) + library.ts (bibliothèque SQLite, Story 5.1, ADR-007)
│   ├── src/preload/        # contextBridge expose window.mixdeck (méthodes Engine API Deck/Mixer/Plugins + bibliothèque + pickFile/pickPluginFile + commandes fenêtre + onEvent)
│   ├── src/renderer/src/   # controllers/ (DeckController/MixerController/PluginController/MasterBusController/LibraryController), state/ (useDeckState, événementiel depuis 2.6), components/ (TitleBar/Deck/FilterKnob/Crossfader/ConsoleMaster, Story 2.5 ; PluginChainPanel, Story 4.3 ; LibraryPanel, Story 5.1) — bridge/ encore vide
...
│   ├── src/                # Deck.cpp (1.1 + 1.4 pitchResampler + 2.1 DeckState/unloadTrack/pause/seek), Mixer.cpp (1.2), FilterDSP.cpp (1.3), Engine.cpp (2.2, graphe audio headless), NodeBinding.cpp (2.2, Bridge N-API) — TimeStretch/BpmAnalyzer (Epic 3), PluginHost/PluginChain/MasterBus/IsolatedPluginHost/PluginWorkerProtocol (Epic 4) ajoutés depuis
│   ├── plugin-worker/      # MixDeckPluginWorker — process dédié à l'isolation VST3 (Story 4.4.1, juce::ChildProcessWorker)
...
**Prochaine action (voir `progress.md`)** : Epic 6 (build local) terminé, périmètre réduit à la Story 6.1 — **toutes les epics fonctionnelles de la roadmap sont désormais achevées**. Prochaines étapes à la main de Julien, au fil de l'usage quotidien : section "Addon" de `roadmap.md` (signature ad-hoc, script one-liner, raccourcis clavier, loop, intégration Apple Music, Readme, etc.), pas de story fonctionnelle en attente.
