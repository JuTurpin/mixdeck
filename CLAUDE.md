...
**Epic 5 ("bibliothèque de sons") en cours** : Story 5.1 (base SQLite + scan/import de dossiers, ADR-007) **validée** — entièrement côté Node/Electron (`apps/electron-ui/src/main/library.ts`), aucun fichier C++ touché (pas de traitement de signal). `better-sqlite3` (reconstruit contre l'ABI Electron via `@electron/rebuild`, voir commande plus bas) + `music-metadata` (ESM-only, import dynamique depuis le process main en CJS). Périmètre minimal : chemin/titre/artiste/durée uniquement (`LibraryPanel.tsx`, tableau + boutons "→ A"/"→ B" réutilisant `DeckController.loadTrack`) — tags/crates/cue points/recherche sont la Story 5.2, pas encore commencée.
...
**Prochaine action (voir `progress.md`)** : Epic 5 (bibliothèque de sons), Story 5.1 (base SQLite + scan/import) validée — passer à la Story 5.2 (métadonnées, tags, crates, cue points).
